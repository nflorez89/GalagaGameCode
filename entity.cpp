...
//=============================================================================
bool Entity::bounce(VECTOR2 &collisionVector, Entity &ent)
{
...
//=============================================================================
// Force of gravity (constant, not cumulative like gravityForce) on this entity from other entity
// Adds the constant gravitational force to the velocity vector of this entity
//                    2              2
//  r*r  =   (Ax - Bx)   +  (Ay - By)
//=============================================================================
void Entity::gravityConstantForce(Entity *ent, float frameTime)
{
    // if either entity is not active then no gravity effect
    if (!active || !ent->getActive())
        return ;
    ...
    Graphics::Vector2Normalize(&gravityV);
    // Multipy by force of gravity to create gravity vector
    gravityV *= entityNS::FORCE * frameTime;
    // Add gravity vector to moving velocity vector to change direction
    velocity = gravityV;
}