...
Ship::Ship() : Entity()
{
    ...
    engineOn = false;
	forwardThrustOn = false;
	leftThrustOn = false;
	rightThrustOn = false;
	backwardThrustOn = false;
    shieldOn = false;
    explosionOn = false;
    mass = shipNS::MASS;
}
                            ...
                            TextureManager *textureM)
{
    ...
    shield.setLoop(true);                  // do loop animation
...
void Ship::update(float frameTime)
{
    ...
    if(forwardThrustOn && !explosionOn)
    {
        velocity.y -= shipNS::SPEED * frameTime;
    }
	if(leftThrustOn && !explosionOn)
	{
		velocity.x -= shipNS::SPEED * frameTime;
	}
	if(rightThrustOn && !explosionOn)
	{
		velocity.x += shipNS::SPEED * frameTime;
	}
	if(backwardThrustOn && !explosionOn)
	{
		velocity.y += shipNS::SPEED * frameTime;
	}
	...
	// make sure ship does not go beyond the edges
